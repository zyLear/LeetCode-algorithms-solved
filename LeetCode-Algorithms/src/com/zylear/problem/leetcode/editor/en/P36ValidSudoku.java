...
// 
//
// Related Topics Array Hash Table Matrix 👍 8841 👎 923
...
public class P36ValidSudoku {
    ...
    class Solution {
        public boolean isValidSudoku(char[][] board) {
            ...
            Set<Character>[] squareSets = new Set[9];
            for (int i = 0; i < 9; i++) {
            ...
            for (int i = 0; i < board.length; i++) {
                for (int j = 0; j < board.length; j++) {
                    char c = board[i][j];
                    if ('.' != c) {
                        if(rowSets[i].contains(c)||columnSets[j].contains(c)||squareSets[(i / 3) * 3 + j / 3].contains(c)){
                            return false;
                        }
                        rowSets[i].add(c);
                        columnSets[j].add(c);
                        squareSets[(i / 3) * 3 + j / 3].add(c);
